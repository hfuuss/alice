...
WorldModel::updateOurDefenseLine()
{
    ...
    int firsr_count=1000; 
    int second_count=1000;
              ...
              ++it )
        {
            ...
            if ( x < second )
            {
                ...
                if ( second < first )
                {
                    ...
                    std::swap( firsr_count,second_count);
                }
            }
        }
    }
    ...
    M_our_defense_line_count=second_count;
    ...
    {
        new_line = ball().pos().x;
        M_our_defense_line_count=ball().posCount();
    }
...
WorldModel::getPlayerNearestTo(const Vector2D &point, const AbstractPlayerCont &players,
                               const int count_thr, double *dist_to_point) const
{

    const AbstractPlayerObject * p = static_cast<AbstractPlayerObject * >( 0 );
    double min_dist2 = 40000.0;

    const AbstractPlayerCont::const_iterator end = players.end();
    for ( AbstractPlayerCont::const_iterator it = players.begin();
          it != end;
          ++it )
    {
        if ( (*it)->posCount() > count_thr )
