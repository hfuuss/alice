...
//    }
    ...
    const double exponent=ServerParam::i().tackleExponent();

    const double x=player2ball.absX();
    const double y=player2ball.absY();
...
//                                         ServerParam::i().tackleExponent() ) );
        ...
        tackle_fail_prob=bound(
            0.0,
            pow(x / tackle_dist, exponent) + pow(y / tackle_width, exponent),
            1.0
          );
