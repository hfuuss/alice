...
//    }
    ...
    //std::cout<<M_chain_graph.getAllChain().size()<<"\n";
...
bool  Bhv_ChainAction::doShoot(PlayerAgent *agent)
{
...
bool  Bhv_ChainAction::ClearDangerousBall(PlayerAgent *agent)
{

    const WorldModel &wm =agent->world();
    if (!ServerParam::i().ourPenaltyArea().contains(wm.ball().pos()))
       {
        return false;
       }
    ...
    if (opp_min<=self_min+3)
    {
        ...
        if (Body_ClearBall().execute(agent)) return true;
    }
