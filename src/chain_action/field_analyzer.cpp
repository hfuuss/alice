...
bool  
 FieldAnalyzer::sameside(const Vector2D pos1, const Vector2D pos2)
{
...
int FieldAnalyzer::estimate_min_reach_cycle(const Vector2D &player_pos,
...
double  FieldAnalyzer::get_opponent_dist(const PredictState &state, const Vector2D &point, int opponent_additional_chase_time, int valid_opponent_thr)
{
    ...
    static const double MAX_DIST = 65535.0;
        ...
        for (AbstractPlayerCont::const_iterator it = state.theirPlayers().begin(),
