...
void
StrictCheckPassGenerator::createVoroniPass( const rcsc::WorldModel & wm ,
                                            const Receiver & receiver )
{
...
//                             : 1 );
    ...
    const  VoronoiDiagramOriginal &voronoi=voron_point::instance().passVoronoiDiagram(wm);
    ...
    for (VoronoiDiagram::Vector2DCont::const_iterator it=V.begin();it!=V.end();it++)

        {
          ...
          if ( (*it).x<wm.theirDefensePlayerLineX()-1.0&&(*it).x<35.0) continue;

            const AngleDeg angle = (receiver.pos_-(*it) ).th();
            const Vector2D receive_point
                =(*it);
                                             ...
                                             const Receiver & receiver )
{
    ...
    static const int ANGLE_DIVS = 30;//14
...
void
StrictCheckPassGenerator::createRqquestPass(const rcsc::WorldModel &wm,const Receiver & receiver )
{
  static const int MIN_RECEIVE_STEP = 6;
#ifdef CREATE_SEVERAL_CANDIDATES_ON_SAME_POINT
    static const int MAX_RECEIVE_STEP = 35;
        ...
        for ( std::vector< AudioMemory::PassRequest >::const_iterator it = wm.audioMemory().passRequest().begin();
              ...
              ++it )
            {
               if ( it->sender_ == receiver.player_->unum() )
               {
                 ...
                 requested_move_angle = ( it->pos_ - receiver.inertia_pos_ ).th();
                 ++M_total_count;
                 const Vector2D receive_point = it->pos_;
                ...
                int start_step = receiver_step;
              if ( receiver.speed_ > 0.2
                        && ( receiver_vel_angle - requested_move_angle ).abs() < 15.0 )
               {
