...
SamplePlayer::initImpl( CmdLineParser & cmd_parser )
{
    ...
    if (!Evaluation::instance().read("./data/sensitivity.net"))
    {
        std::cerr << "***ERROR*** Failed to read Evaluation." << std::endl;
        return false;
    }
...
//      }
    ...
    //
    // try tackle//or intercept
    //
    if ( ! wm.self().goalie()
         && wm.gameMode().type() == GameMode::PlayOn
         && Bhv_BasicTackle().execute( this ) )
    {
        return true;
    }
...
*/
FieldEvaluator::Ptr
SamplePlayer::createFieldEvaluator()
{
#ifdef  USING_MIRACLE_BRAN
    return FieldEvaluator::ConstPtr(new MiracleBrain);
#else
    return FieldEvaluator::Ptr( new Brain );
#endif
}
...
//                     ( new ActGen_AttackPass(), 2) );
    ...
    //
    g->addGenerator( new ActGen_MaxActionChainLengthFilter
                     ( new ActGen_StrictCheckPass(),pass_depth) );
