...
Bhv_GoalieBasicMove::doOffensivePositioning( PlayerAgent * agent )
{
  ...
  float dashPower;
  double dist_thr = wm.ball().distFromSelf() * 0.03;
    if( dist_thr < 0.8 )
        dist_thr = 0.8;
...
/*
           Body_GoToPoint( M_target_point, dist_thr,
...
*/
    ...
    if ( !Body_GoToPoint( homePos, dist_thr, dashPower ).execute( agent ) )
  ...
  {
    Body_GoToPoint( Vector2D(-50.5,getTargetPoint(agent).y),
...
Bhv_GoalieBasicMove::doOffensiveIntercept( PlayerAgent * agent )
{
  ...
  if( myCycles <= oppCycles - 2 && myCycles < tmmCycles &&
                                       ...
                                       Vector2D &move_point )
{
  ...
  {
     if( ball.x > 25.0 )
       move_point = Vector2D( -22.0,0.0 );
     else if( ball.x > 15.0 )
       move_point = Vector2D( -28.0,0.0 );
     else if( ball.x > 5.0 )
       move_point = Vector2D( -30.0,0.0 );
     else if( ball.x > -5.0 )
       move_point = Vector2D( -32.0,0.0 );
     else if( ball.x > -15.0 )
       move_point = Vector2D( -37.0,0.0 );
     else if( ball.x > -22.0 )
