...
//    //set play mark  dy  2017年 01月 03日 星期二 13:50:09 CST

#ifdef  USING_SET_PLAY_MARKING
  const  AbstractPlayerObject * mark_target = mark_find_best_target(agent).get_set_play_mark_opp();


    if (mark_target)
    {
        AngleDeg mark_angle=(wm.ball().pos()-mark_target->pos()).th();
        const double mark_dist=1.0;
        target_point=mark_target->pos()+Vector2D::polar2vector(mark_dist,mark_angle);
    }
    ...
    {
        double xdiff = std::sqrt( std::pow( 10.0, 2 ) - std::pow( ball_to_target.y, 2 ) );
