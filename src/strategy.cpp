...
const std::string Strategy::D_L_CONF = "D_L.conf";
const std::string Strategy::D_R_CONF = "D_R.conf";


const std::string Strategy::Y_L_CONF = "Y_L.conf";
const std::string Strategy::Y_R_CONF = "Y_R.conf";
...
Strategy::read( const std::string & formation_dir )
{
    ...
    //add
    M_D_L_formation=readFormation(configpath+D_L_CONF);
    if (!M_D_L_formation)
    {
        std::cerr<<"Failed  to read K_L_CONF"<<std::endl;
    }

    M_D_R_formation=readFormation(configpath+D_R_CONF);

    if (!M_D_R_formation)
    {
      std::cerr<<"Failed  to  read K_R_CONF"<<std::endl;
    }


    //add
    M_Y_L_formation=readFormation(configpath+Y_L_CONF);
    if (!M_Y_L_formation)
    {
        std::cerr<<"Failed  to read Y_L_CONF"<<std::endl;
    }

    M_Y_R_formation=readFormation(configpath+Y_R_CONF);

    if (!M_Y_R_formation)
    {
      std::cerr<<"Failed  to  read Y_R_CONF"<<std::endl;
    }
    s_initialized = true;
    return true;
}
...
Strategy::updateSituation( const WorldModel & wm )
{
    ...
    //2017  dy
    const int defensive_cycle_thr=wm.ball().pos().x<0? 0:2;

    if ( opp_min <= our_min - defensive_cycle_thr ||wm.existKickableOpponent())
    {
        dlog.addText( Logger::TEAM,
...
double
Strategy::get_defender_dash_power(const WorldModel &wm, const Vector2D &homePos)
{
    static bool s_recover_mode = false;
     ...
     bool opp_attacck= opp_min<mate_min||wm.existKickableOpponent();
                              ...
                              const rcsc::Vector2D & home_pos )
{
    ...
    const  int opp_min=wm.interceptTable()->opponentReachCycle();
    const  int mate_min=wm.interceptTable()->teammateReachCycle();
    const  int min_min=std::min(opp_min,mate_min);
